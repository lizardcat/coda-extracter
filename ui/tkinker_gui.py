...
try:
    from coda_extractor import CodaTimesheetExtractor
    from data_processor import TimesheetProcessor
except ImportError:
    print("Error: Could not import extractor modules. Make sure you're running from the correct directory.")
    sys.exit(1)
        ...
        if os.path.exists(config_file):
            try:
                with open(config_file, 'r') as f:
                    ...
                    # Don't save API token for security
            ...
            with open(env_file, 'r') as f:
                for line in f:
                    if line.startswith('CODA_API_TOKEN='):
                        self.api_token.set(line.split('=', 1)[1].strip())
                    elif line.startswith('CODA_DOC_ID='):
                        self.doc_id.set(line.split('=', 1)[1].strip())
                    elif line.startswith('CODA_TABLE_ID='):
                        self.table_id.set(line.split('=', 1)[1].strip())
